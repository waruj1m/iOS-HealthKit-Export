...
struct HealthExportService {
    ...
    func generateExportData(format: ExportFormat = .json) async throws -> (Data, String) {
        ...
        switch format {
        case .json:
            return (try generateJSON(records: records, startDate: startDate, endDate: endDate), fileName)
        case .csv:
            return (generateCSV(records: records), fileName)
        case .xlsx:
            return (generateXLSX(records: records), fileName)
        }
    }
    ...
    private func fetchAllRecords(from startDate: Date, to endDate: Date) async -> [HealthRecord] {
        ...
        return await withCheckedContinuation { continuation in
            ...
            let query = HKSampleQuery(
                ...
                let records = samples.map {
                    HealthRecord(type: label, date: ISO8601DateFormatter().string(from: $0.startDate),
    ...
    private func querySleepRecords(start: Date, end: Date) async -> [HealthRecord] {
        ...
        return await withCheckedContinuation { continuation in
            ...
            let query = HKSampleQuery(
                ...
                let records = samples.map {
                    HealthRecord(type: "sleepAnalysis",
    ...
    private func queryMindfulRecords(start: Date, end: Date) async -> [HealthRecord] {
        ...
        return await withCheckedContinuation { continuation in
            ...
            let query = HKSampleQuery(
                ...
                guard let samples = samples as? [HKCategorySample] else { continuation.resume(returning: []); return }
                let records = samples.map {
                    HealthRecord(type: "mindfulSession",
                                 date: ISO8601DateFormatter().string(from: $0.startDate),
    ...
    private func queryWorkoutRecords(start: Date, end: Date) async -> [HealthRecord] {
        return await withCheckedContinuation { continuation in
            let predicate = HKQuery.predicateForSamples(withStart: start, end: end, options: .strictStartDate)
            let query = HKSampleQuery(
    ...
    private func generateJSON(records: [HealthRecord], startDate: Date, endDate: Date) throws -> Data {
    ...
    private func generateCSV(records: [HealthRecord]) -> Data {
    ...
    private func generateXLSX(records: [HealthRecord]) -> Data {
...
// MARK: - XLSX / ZIP Builder (no external dependencies)

extension HealthExportService {

    private func buildXLSX(sheets: [(name: String, rows: [[String]])]) -> Data {
        var entries: [(name: String, data: Data)] = []

        // Shared strings table — collect all string cells up-front
    ...
    private func xlsxCol(_ index: Int) -> String {
    ...
    // MARK: - ZIP Writer (stored, no compression — valid ZIP/XLSX)
